...
float   ballX             = 100 ,  ballY             = 100;  //-- Ændret til float og ikke int
float   eyeLeftRelativeX  = 5   ,  eyeLeftRelativeY  = 5;    //-- Ændret til float og ikke int
float   eyeRightRelativeX = 20  ,  eyeRightRelativeY = 5;    //-- Ændret til float og ikke int
...
void draw() {                         
  ...
  ballY         =  ballY + ballwalk +  speedY; //--- Vi sætter ballwalk ind her for at få den til at komme op og ned af y-aksen. Og ikke til hen af x-aksen.
  ...
  clear();
  ellipse(ballX, ballY, ballSize, ballSize); // bolden
